...
export default function ProductsCarousel({ productsData }) {
  if (
    !productsData ||
    !productsData.product_articles ||
    productsData.product_articles.length === 0
  ) {
    return null;
  }
  ...
  return (
    <section className="bg-white py-12 px-4">
      <div className="max-w-6xl mx-auto text-center mb-12 text-3xl text-gray-800">
        <h2>{productsData.title || "Des créations artisanales uniques"}</h2>
        <p className="mt-2">
        ...
        {productsData.product_articles.map((product, index) => {
          ...
          return (
            <ShopCard
              ...
              productId={product.documentId || index}
