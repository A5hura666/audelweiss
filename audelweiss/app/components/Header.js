...
export default function Header() {
    ...
    const fetchHeaderData = async () => {
      try {
        const response = await fetch(
          getStrapiCall(
            "/api/global?populate[0]=header&populate[1]=header.iconsLinks&populate[2]=header.iconsLinks.icon&populate[3]=header.links&populate[4]=header.links.MegaMenu&populate[5]=header.links.MegaMenu&populate[7]=header.links.MegaMenu.productLinks.image&populate[6]=header.links.MegaMenu.productLinks"
          )
        );
  ...
  return (
    <header
      className={`fixed z-50 top-0 left-0 w-full bg-white transition-transform duration-300 ${
            ...
            >
              <Link
                ...
                className={`text-sm font-bold ${
                  pathname === item.url ? "text-[#E8A499]" : "text-gray-700"
