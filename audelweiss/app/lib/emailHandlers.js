...
class WelcomeEmailHandler extends BaseEmailHandler {
  validate(data) {
...
class OrderConfirmationHandler extends BaseEmailHandler {
  validate(data) {
    const { email, name } = data;
  ...
  async execute(data) {
    const { email, name } = data;
    return await emailService.sendOrderConfirmation(email, name);
  }
}
...
class EmailHandlerFactory {
   ...
   * @private
   */
  static #handlers = new Map([["contact", ContactNotificationHandler]]);
  ...
  static createHandler(type) {
    ...
    if (!HandlerClass) {
      ...
      throw new Error(
        `Type d'email non supporté: ${type}. Types supportés: ${supportedTypes}`
      );
    }
