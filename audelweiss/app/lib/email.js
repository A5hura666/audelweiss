...
export const emailService = {
  ...
  async sendEmail({ to, subject, text, html, replyTo, attachments }) {
    try {
      ...
      throw new Error(
        `Échec de l'envoi de l'email vers ${to}: ${error.message}`
      );
    }
  },
  ...
  async sendContactNotification(to, contactData) {
    ...
    if (missingFields.length > 0) {
      throw new Error(
        `Champs manquants dans le formulaire de contact: ${missingFields.join(
          ", "
        )}`
      );
    }
  ...
  async sendOrderConfirmation(to, name) {
    const html = orderConfirmationTemplate(name);
    return this.sendEmail({
      to,
  ...
  getSupportedEmailTypes() {
    return {
      types: ["contact"],
      templates: ["contactNotificationTemplate"],
      description:
        "Service d'email simple pour les notifications de contact Audelweiss",
    };
  },
};
