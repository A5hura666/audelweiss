...
export default function Product() {
    ...
    const fetchHeaderData = async () => {
      try {
        const response = await fetch(
          getStrapiCall(
            `/api/product-article-descriptions?filters[product_article][documentId][$eq]=${idArticle}` +
        ...
        setProductPrice(data.data[0].product_article.price);
        ...
        setProductImages(
          data.data[0].product_article.productImages.map(
            (image) => `http://ayun.myddns.me:5000${image.url}`
          )
        );
        ...
        const productRecommandationResponse = await fetch(
          getStrapiCall(
            `/api/product-article-cards?populate=productImages&pagination[pageSize]=5&pagination[page]=1`
          )
        );
        const productRecommandationData =
          await productRecommandationResponse.json();
  ...
  const handleFilterChange = (filterName, value) => {
    ...
    if (filterName === "Taille") {
      setProductPrice(
        value === "enfant" ? productChildPrice : productParentPrice
      );
    }
  };

  const addToCart = () => {
    const allFiltersFilled = Object.values(selectedFilters).every(
      (value) => value !== ""
    );
    if (!allFiltersFilled) {
      Swal.fire({
  ...
  return (
    <section className="">
      ...
      <section
        ...
        className={"px-4 2xl:px-[20%] xl:px-[10%] lg:px-[5%] py-4"}
      >
        <div
          className={
            "flex flex-col md:flex-row md:items-start justify-center text-center py-[20px] w-full gap-4"
          }
        >
          <section
            className={
              "flex flex-col items-start justify-center text-center py-[20px] w-full"
            }
            style={{ width: `100%` }}
          >
            <section className="flex flex-col gap-4 mx-auto">
              <div>
                <img
                  ...
                  className={
                    "h-auto max-w-full rounded-lg border-4 border-white shadow-lg cursor-pointer max-h-[650px]"
                  }
            ...
            <i className={" hidden md:block w-full text-center py-4"}>
              Sur ordinateur, utilises les flèches du clavier pour naviguer
              entre les photos 😊
            </i>
          </section>
          <section
          ...
          >
            ...
            <section className={"flex flex-col gap-4 list-none"}>
              <p className={"w-5/6 leading-7"}>
                {productDescription ||
                  "Aucun descriptif disponible pour ce produit."}
              </p>
              {productPrice != null ? (
                <h4 className="text-[#ff6187] text-3xl">{productPrice} €</h4>
              ) : (
                <h4 className="text-[#ff6187] text-3xl">
                  {productChildPrice} € - {productParentPrice} €
                </h4>
              )}
              <div>
                        ...
                        {Object.entries(filterValues).map(([key, val]) => (
                          <li key={key}>
                            <input
                              ...
                              onChange={() =>
                                handleFilterChange(filterName, val)
                              }
              ...
              <section>
                ...
                {productOffers && (
                  <section
                    className={
                      "text-lg text-gray-500 mt-2 bg-[#fbedec] p-2 rounded flex flex-col gap-2"
                    }
                  >
        ...
        <div
          className={
            "flex flex-col md:flex-row space-between items-start justify-between text-center gap-12 py-[20px] w-full"
          }
        >
          <section
            className={"w-[100%] md:w-[70%] text-left flex flex-col gap-4"}
          >
            <MarkdownRenderer markdownText={markdown} />
          </section>
          <section
            className={
              "w-[70%] md:w-[30%] items-center md:items-start text-left flex flex-col gap-4 mx-auto"
            }
          >
            ...
            <table className={"w-full"}>
              <tbody className={""}>
                <tr className={"border-pink leading-[50px]"}>
                  <th className={"w-[15%]"}>
                    <img
                      src="https://audelweiss.fr/wp-content/uploads/2025/02/yarn.svg"
                      alt=""
                    />
                  </th>
                ...
                <tr className={"border-pink leading-[50px]"}>
                  <th>
                    <img
                      src="https://audelweiss.fr/wp-content/uploads/2025/02/washing-machine.svg"
                      alt=""
                    />
                  </th>
                  <td>
                    Lavage en machine :{" "}
                    {productInformations.washingMachine ? "Oui" : "Non"}
                  </td>
                </tr>
                {productInformations.washingMachine && (
                  <tr className={"border-pink leading-[50px]"}>
                    <th>
                      <img
                        src="https://audelweiss.fr/wp-content/uploads/2025/02/temperature-2.svg"
                        alt=""
                      />
                    </th>
                    <td>
                      Température max de lavage :{" "}
                      {productInformations.maxWashingTemperature}°C
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      >
        <div className="flex flex-wrap items-center justify-center gap-6 mb-3 text-sm text-gray-800 dark:text-gray-200">
          <span>
            <strong>Couleur :</strong> {color || "Aucune couleur sélectionnée"}
          </span>
          <span>
            <strong>Quantité :</strong> {quantity}
          </span>
          <span>
            <strong>Total :</strong> {(productPrice * quantity).toFixed(2)} €
          </span>
        </div>
